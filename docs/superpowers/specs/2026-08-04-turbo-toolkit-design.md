...
### 1.3 `prettier-config`·`tsconfig`의 테스트는 `tsc`로 검사된 적이 없다

두 패키지는 TS 소스가 없어 `tsconfig.json`도 없다. 그런데 테스트는 `.ts`다. 지금까지 그 파일들을 파싱한 것은 ESLint의 `projectService`뿐이고, **타입 검사는 한 번도 돌지 않았다.** 4.3절에서 메운다.
...
### 4.3 `prettier-config`·`tsconfig`의 테스트 타입 검사

1.3절의 해소다. 두 패키지에 `tests/tsconfig.json`을 신설해 `typecheck` 대상에 넣는다. 지금까지 검사되지 않던 코드가 검사에 들어오므로 **이 단계에서 기존 타입 오류가 드러날 수 있다** — 그것은 이 작업의 실패가 아니라 발견이다.
