...
#### 그 대가 — 템플릿 JSON은 정규형이어야 한다 (e2e가 실측으로 잡음)

환원은 **직렬화 비대칭**을 만든다. `create`는 템플릿 JSON을 **원문 텍스트 그대로** 쓰지만(손으로 압축한 `"include": ["src", "test"]`가 보존된다), `update`는 전부 `JSON.stringify(…, null, 2)`로 **재직렬화**한다(배열이 항상 여러 줄로 펼쳐진다). 의미는 같고 바이트만 다르다.

결과는 침묵이 아니라 잡음이다 — **갓 생성한 프로젝트에 `update`를 돌리면 아무것도 바뀌지 않았는데 "덮어쓰기"가 뜬다.** 10절 5층 e2e가 정확히 이것을 잡았다(`nest`의 `tsconfig.json`, `monorepo`의 `turbo.json` 두 건).

**해소는 두 경로가 같은 바이트를 만들게 하는 것이다**: 모든 템플릿 `.json`을 정규형(`JSON.stringify(JSON.parse(t), null, 2) + '\n' === t`)으로 유지한다.

대안이었던 *"패치가 없는 순수 JSON 오버레이는 원문 비교로 우회한다"* 는 기각했다. 그 우회는 JSON 오버레이를 다시 원문으로 다루는 것이라 **바로 위에서 고친 버그(사용자의 `compilerOptions.paths` 소실)를 되살린다.** 갈라짐을 없애는 대신 비교 시점에 가리는 해법이라 근본 비대칭도 남는다.

**드리프트 방어는 테스트다.** 템플릿을 손으로 맞추는 것만으로는 부족하다 — 누군가 배열을 다시 압축하면 같은 문제가 돌아오고, 그때는 **느리고 비싼 e2e에서만** 드러난다. `templates/**/*.json`이 전부 정규형인지 단위 테스트가 단언한다.
