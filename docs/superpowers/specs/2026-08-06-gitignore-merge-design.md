...
- **줄 삭제가 전파되지 않는다.** 템플릿에서 규칙을 지워도 기존 프로젝트에는
  ...
  가능하다. **단, 블록이 되살리려는 경로의 조상 제외 줄은 예외다.** `!X/Y/`
  처럼 블록이 특정 하위 경로를 되살리려는데 대상에 그 조상(`X/`·`X`)을
  통째로 무시하는 줄이 이미 있으면, git 은 애초에 그 디렉토리 안으로 내려가지
  않아 블록이 물리적으로 동작할 수 없다(git-ignore(5)) — "남는 쪽이 회복
  가능하다"는 근거가 이 경우엔 성립하지 않는다(남은 줄이 새 기능을
  무력화한다). 정확히 일치하는 조상 줄만 지운다.
