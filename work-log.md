...
- **내용**: 소비자(my-blogs)가 devkit을 실제로 도입하며 낸 리포트 6건을 devkit 실물에 대고 검증하고 전부 반영했다. 커밋 `7fd3046`·`0d1c69a`·`b4d157d`·`c34e39e`.
  1. **리포트 1건은 원인이 틀렸다.** "`@next/eslint-plugin-next`가 `eslint-plugin-react@7.37.5`·`eslint-plugin-import`를 물고 온다"는 서술이 있었으나, 16.2.12는 **peerDependencies가 없고 의존성이 `fast-glob` 하나뿐**이다. 실제 출처는 소비자에 남아 있던 `eslint-config-next`(16.3.0의 `dependencies`에 `eslint-plugin-react ^7.37.0`·`eslint-plugin-import ^2.32.0`, `npm view`로 확인). 이름이 비슷해 devkit 프리셋 탓으로 오인되기 쉬운 자리라, 고칠 대상 자체가 달라진다(프리셋이 아니라 도입 가이드). 루트 README에 둘을 구분해 적었다.
