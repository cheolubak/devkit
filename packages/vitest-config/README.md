...
소비자 `package.json`에는 `link:` 의존으로 추가하고, `vitest`는 peerDependency이므로 소비자가 직접 설치한다. `next` 프리셋은 `jsdom`도 요구하므로(아래 참고) 함께 설치한다.
...
ESM 순수 객체이므로 트랜스파일할 것이 없다. `link:` 의존은 라이프사이클 스크립트를 실행하지 않으므로, 빌드가 필요한 패키지는 `dist`가 낡으면 소비자가 조용히 옛 설정을 쓰게 된다. 이 패키지는 그 문제를 아예 겪지 않는다.
