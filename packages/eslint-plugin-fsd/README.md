...
```bash
pnpm add -D @devbak/eslint-plugin-fsd
```
...
```js
// eslint.config.js
import fsd from '@devbak/eslint-plugin-fsd';
...
```js
// Next.js 앱
import fsdNext from '@devbak/eslint-plugin-fsd/next';
...
```js
// 순수 React 앱
import fsdReact from '@devbak/eslint-plugin-fsd/react';
...
| 서브패스 | 필요한 패키지 |
|---|---|
| `@devbak/eslint-plugin-fsd` | 없음 |
| `@devbak/eslint-plugin-fsd/react` | `eslint-plugin-react-hooks` |
| `@devbak/eslint-plugin-fsd/next` | 위 1개 + `eslint-plugin-jsx-a11y`, `@next/eslint-plugin-next` |
