...
```bash
pnpm add -D @cheolubak/eslint-config-nest eslint typescript-eslint eslint-plugin-zod zod
```
...
이 패키지 자체가 끄는 규칙은 정확히 **하나**, `@typescript-eslint/unbound-method`다. 완화 범위는 `*.spec.ts`·`*.e2e-spec.ts` 파일명 패턴으로만 한정된다 — `*.test.ts` 명명 규칙을 쓰는 프로젝트에서는 이 규칙이 그대로 발화한다.
